...
class AlphaIntegration:
    ...
    def sequential_integration(self, x: np.ndarray, k: int) -> float:
        ...
        logging.debug(f"Sequential Integration (k={k}): result = {result:.6f}, error = {error:.6e}")
        return result
    ...
    def compute_uai(self, s_values: np.ndarray) -> float:
        ...
        logging.debug(f"UAI 계산: result = {result:.6f}, error = {error:.6e}")
        return result
...
class YangMillsSimulator:
    ...
    def string_tension(self) -> float:
        ...
        logging.debug(f"String Tension: sigma = {sigma:.6f}, error = {error:.6e}")
        return m**2 * (1 + sigma / m)

    def mass_gap(self) -> float:
        sigma = self.string_tension()
        E_0 = np.sqrt(sigma)
        logging.info(f"Mass Gap: E_0 = {E_0:.6f}")
        return E_0
    ...
    def compute_D_A_squared_gpu(self, x: cp.ndarray, alpha: float, beta: float) -> cp.ndarray:
        ...
        logging.info(f"샘플 처리 시작: {N_samples} 샘플")
                ...
                for i in range(3):
                    for j in range(3):
                        for a in range(self.n_generators):
                            ...
                            A = alpha * self.lambda_qcd * cp.exp(-beta * r) * self.generators_cp[a]
                            ...
                            for b in range(self.n_generators):
                                for c in range(self.n_generators):
                                    ...
                                    A_i_b = alpha * self.lambda_qcd * cp.exp(-beta * r) * self.generators_cp[b]
                                    A_j_c = alpha * self.lambda_qcd * cp.exp(-beta * r) * self.generators_cp[c]
                                    commutator += f_val * (A_i_b @ A_j_c)
                            D_A = dA + self.g * commutator
                            D_A_sq_temp += cp.sum(cp.abs(D_A)**2)
                D_A_sq[start + idx] = D_A_sq_temp
            logging.info(f"배치 완료: {end}/{N_samples}, 경과 시간: {time.time() - start_time:.2f}초")
    ...
    def gribov_parameter(self) -> float:
        def D_A_squared(params: np.ndarray):
            alpha, beta = params
            L = 1.0  # GeV^-1
            N_samples = 1000000  # CuPy로 10^6 샘플 처리
            x = cp.random.uniform(-L/2, L/2, (N_samples, 4)).astype(cp.float64)
            
            logging.info("CuPy GPU 계산 활성화")
            D_A_sq = self.compute_D_A_squared_gpu(x, alpha, beta)
            result = cp.mean(D_A_sq) / (L**4)
            logging.info(f"GPU 계산 완료: 평균 D_A^2 = {result:.6f}")
            return float(result.get())

        try:
            result = optimize.minimize(D_A_squared, x0=[1.0, 1.0], bounds=[(0, 10), (0, 10)], 
                                     method='L-BFGS-B', options={'maxiter': 1000, 'disp': False})
            gamma_sq = result.fun
            gamma = np.sqrt(gamma_sq)
            logging.info(f"Gribov Parameter: gamma = {gamma:.6f}, Optimization Success: {result.success}")
            return gamma
    ...
    def wilson_loop(self, L: float, T: float) -> float:
        ...
        logging.debug(f"Wilson Loop (L={L}, T={T}): W = {W:.6f}")
